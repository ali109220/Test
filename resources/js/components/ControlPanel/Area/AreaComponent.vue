<template>
    <div id="app" class="container">
        <AddAreaComponentVue></AddAreaComponentVue>
    <div style="overflow-x:auto;">
        <table class="table" style="table-layout: fixed;border-collapse: collapse;
                        ...
                        <div class="popup">
                            ...
                            <div class="popuptext popUpFilter" :id="col.name"  style="width: 185px;height: 140px;left: -175px;">
                                <div class="dropdown">
                                    ...
                                    <input v-if="!col.number&&!col.isDate&&!col.isTime&&!col.isBoolean&&!col.isReference" :id="col.name+col.type" type="text" class="form-control" style="margin-left: 30px;width: 125px; margin-top: 6px;height: 32px;">
            ...
            <tbody>
                <tr v-for="area in areas" v-bind:key="area.id" @dblclick="ShowInfo(area.id)">
                    <th scope="row" style="text-align: left;padding: 8px;">{{area.id}}</th>
                    <td v-for="col in columns" v-bind:key="col.name" style="text-align: left;padding: 8px;word-wrap:break-word">{{getValueOfCol(col.name,area.id)}}</td>
                </tr>
            </tbody>
        </table>
        </div>
        <nav aria-label="Page navigation example">
           <ul class="pagination">
               <li v-bind:class="[{ disabled : !pagination.prev_page}]" class="page-item"><a class="page-link" @click="fetchAreas(pagination.prev_page)">Previous</a></li>
               <li class="page-item disabled"><a class="page-link">{{pagination.current_page}} of {{pagination.last_page}}</a></li>
               <li v-bind:class="[{ disabled : !pagination.next_page}]" class="page-item"><a class="page-link" @click="fetchAreas(pagination.next_page)">Next</a></li>
           </ul>
        </nav>
    </div>
</template>
<script>
import AddAreaComponentVue from './AddAreaComponent'
    export default {
        data(){
            return{
                filters :[],
